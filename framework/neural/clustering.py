...
def cluster(guesses,n_clusters):
	#FILL IN HERE
...
guesses = np.average(y_train_arr,axis=0)
for iterations in xrange(0,10):
	...
	total_2 = 0
	n_clusters = 3
	mapping = cluster(guesses,n_clusters)
	for clust in xrange(0,n_clusters):
		...
		num_el = 0
		for i in xrange(0,np.shape(mapping)[0]):
			if mapping[i] == clust:
				num_el += 1
				if single_y_train == None:
					single_y_train = y_train_arr[:,i:i+1]
				else:
					single_y_train = np.append(single_y_train, y_train_arr[:,i:i+1], axis=1)

				if single_y_test == None:
					single_y_test = y_test_arr[:,i:i+1]
				else:
					single_y_test = np.append(single_y_test, y_test_arr[:,i:i+1], axis=1)

				if single_y_valid == None:
					single_y_valid = y_valid_arr[:,i:i+1]
				else:
					single_y_valid = np.append(single_y_valid, y_valid_arr[:,i:i+1], axis=1)

		print np.shape(x_train_arr)
		print np.shape(single_y_train)
		print num_el
		model = Sequential()
		model.add(Dense(input_dim=67, output_dim=num_el, activation='linear'))
		model.add(Dense(input_dim=num_el, output_dim=num_el, activation='linear'))
		...
		model.fit(x_train_arr, single_y_train, batch_size=100, nb_epoch=8, shuffle=True, validation_data=(x_valid_arr, single_y_valid))
	...
	print "-------------Results for iteration 1_____________"
